...
To ensure robustness and scalability, we avoided creating a new thread for every single connection, which could lead to resource exhaustion (DoS). Instead, we utilized a \textbf{Thread Pool} pattern.

\begin{itemize}
    \item We used \texttt{java.util.concurrent.ExecutorService} with a fixed pool size (configurable via command line arguments, typically 10 threads).
    \item When a socket is accepted, the job is submitted to the pool. If all threads are busy, the request waits in a queue.
\end{itemize}

\textbf{Data Synchronization:} Since multiple threads (IMAP and SMTP) might access the same mailbox simultaneously, we synchronized critical sections in \texttt{MailStorageManager}. Methods modifying the file system or the metadata files (e.g., \texttt{saveEmail}, \texttt{updateFlag}, \texttt{getNextUID}) are marked with the \texttt{synchronized} keyword. This guarantees that file writes are atomic and prevents race conditions where two emails could receive the same UID.
